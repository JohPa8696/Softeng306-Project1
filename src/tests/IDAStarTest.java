...
public class IDAStarTest extends TestCase {
	...
	public void testDisjointGraphs() throws InvalidArgumentException, FileNotFoundException {
		...
		ip.processInput();
		IDAStar s = new IDAStar(ip.getListOfNodes(), ip.getNextAvailableNodes(), ip.getNumberOfProcessors());
		s.schedule();
	...
	/*
	 * @Test TAKING TOO LONG TO BE INCLUDED public void testNodes11() throws
	 * InvalidArgumentException, FileNotFoundException{ String
	 * args[]={"resources/Nodes_11_OutTree.dot","3"}; InputProcessor ip= new
	 * InputProcessor(args); ip.processInput(); IDAStar s=new
	 * IDAStar(ip.getListOfNodes(), ip.getNextAvailableNodes(),
	 * ip.getNumberOfProcessors(), ip.getNumThread()); s.schedule(); int
	 * finishTime=s.getFinishTime(); System.out.println(finishTime);
	 * assertEquals(finishTime,104); }
	 */
	@Test
