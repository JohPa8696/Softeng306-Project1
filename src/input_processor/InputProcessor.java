...
public class InputProcessor implements TaskReader {
	...
	public void processInput() throws FileNotFoundException {
		...
		while (scan.hasNext()) {
			...
			// Process lines that do not contain ">" characters, representing
			// nodes
			if (!line.contains(">")) {
				...
				if (isNodeNew(name)) {
					// If the node is not in the list_of_nodes then create a new
					// node and add to the list
					...
					// If the node is already in the list, assign the weight to
					// the node
				...
				Node p = null;
				// If the parent node does not exist create a new node,
				// otherwise get the parent node and assign to p
				...
				// Get the child node
				Node c = null;
				// If the child node does not exist create a new node, otherwise
				// get the child node and assign to c
